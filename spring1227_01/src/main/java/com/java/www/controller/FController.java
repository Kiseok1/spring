...
public class FController {
	...
	public String searchData(String txt) throws Exception {
		...
		if(txt.equals("") || txt==null) {
			//사진목록 메소드 호출 - 검색단어가 없을 때
			result = gallaryList(page, serviceKey);
		} else {
			//사진조회 메소드 호출 - 검색단어가 있을 때
	...
	public String searchScreen(String txt) throws Exception {
		...
		if(txt.equals("") || txt==null) {
			//사진목록 메소드 호출 - 검색단어가 없을 때
			result=boxoffice(page, serviceKey);
		} else {
			//사진조회 메소드 호출 - 검색단어가 있을 때
			result=boxofficeSearch(txt, page, serviceKey);
		}
	...
	//screenData 검색안함
	public String boxoffice(String page, String serviceKey) throws Exception {
	...
	//screenData 검색함
	public String boxofficeSearch(String txt, String page, String serviceKey) throws Exception {
		StringBuilder urlBuilder = new StringBuilder("https://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json"); /*URL*/
		urlBuilder.append("?" + URLEncoder.encode("key","UTF-8") + "="+serviceKey); /*Service Key*/
		urlBuilder.append("&" + URLEncoder.encode("targetDt","UTF-8") + "=" + URLEncoder.encode(txt, "UTF-8")); /*목록 건수*/
		URL url = new URL(urlBuilder.toString());
		//URL url = new URL("https://apis.data.go.kr/B551011/PhotoGalleryService1/galleryList1?serviceKey=918RE13GA7OY7ZEmUzApgbOeAcQoZ%2FaHsXWcqPAKQ9YNNPj83KOstRMRIUrCFIAcm9qj2R6b7NFZjp%2FYsYzJLg%3D%3D&numOfRows=10&pageNo=2&MobileOS=ETC&MobileApp=AppTest&arrange=A&_type=json");
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("GET");
	...
	public String searchBike(String txt) throws Exception {
		...
		if(txt.equals("") || txt==null) {
			//사진목록 메소드 호출 - 검색단어가 없을 때
			result=bikeData(page, serviceKey);
		} else {
			//사진조회 메소드 호출 - 검색단어가 있을 때
			result=boxofficeSearch(txt, page, serviceKey);
		}
	...
	public String bikeData(String page, String serviceKey) throws Exception {
		StringBuilder urlBuilder = new StringBuilder("http://openapi.seoul.go.kr:8088/"); /*URL*/
        urlBuilder.append(serviceKey+"/"); /*Service Key*/
        urlBuilder.append("json/"); 
        urlBuilder.append("bikeList/"); 
        urlBuilder.append("1/"); 
        urlBuilder.append("5/"); 
